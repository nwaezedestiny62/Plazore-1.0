...
export default function OrderDetailPage() {
  ...
  const onConfirmDelivery = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE}/orders/${order._id}/confirm-delivery`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      if (!res.ok || json.success === false) {
        throw new Error(json.message || "Could not confirm delivery");
      }
      if (json.data) setOrder(json.data);
      else await loadOrder();
    } catch (e: any) {
      setActionError(e?.message || "Could not confirm delivery");
    } finally {
      setActionBusy(false);
    }
  };
  ...
  const onIssues = async () => {
    ...
    try {
      const token = await getToken();
      const res = await fetch(
        `${BASE}/orders/${order._id}/report-delivery-issue`,
        {
          method: "PUT",
          headers: {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok || json.success === false) {
        throw new Error(json.message || "Could not open issue");
      }
      if (json.data) setOrder(json.data);
      ...
      setActionError(e?.message || "Could not open issue");
    } finally {
      setActionBusy(false);
    }
  };
  ...
  if (!order) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-bg text-center text-text">
        ...
        <button
          onClick={() => router.back()}
          className="mt-4 rounded-xl border border-white/7 bg-surface px-5 py-2.5 text-sm font-semibold"
        >
          Go back
        </button>
      </div>
        ...
        <div>
          ...
          {needsConfirm && (
            <div className="mb-3 rounded-2xl border border-[#00E575]/25 bg-[#00E575]/[0.06] p-4">
              ...
              {actionError ? (
                <p className="mt-3 text-[12px] text-red-400">{actionError}</p>
              ) : null}
              <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                <button
                  ...
                  className="h-11 rounded-xl bg-[#00E575] px-5 text-sm font-semibold text-[#041412] disabled:opacity-60"
                >
                  {actionBusy ? "Working…" : "Confirm delivery"}
                </button>
                <button
                  type="button"
                  disabled={actionBusy}
                  onClick={onIssues}
                  className="h-11 rounded-xl border border-white/12 bg-surface-2 px-5 text-sm font-semibold text-secondary disabled:opacity-60"
                >
                  Issues?
                </button>
              </div>
          ...
          {issueOpen && (
            <div className="mb-3 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4">
              ...
              <button
                ...
                className="mt-3 text-[13px] font-semibold text-[#00E575]"
              >
                Open Contact
              </button>
            </div>
          ...
          {confirmed && (
            <div className="mb-3 rounded-2xl border border-[#00E575]/20 bg-[#00E575]/[0.05] p-4">
                    ...
                    <div className="mr-3 flex w-[30px] flex-col items-center">
                      {isActive ? (
                        <span
                          ...
                          style={{
                            backgroundImage:
                              "linear-gradient(135deg,#00E575,#3B82F6)",
                          }}
                        >
          ...
          {hasShippingBlock && (
            <div className="mb-3 rounded-2xl border border-white/7 bg-surface p-4">
              ...
              <div className="mb-3.5">
                <p className="mb-1 text-[11px] font-semibold uppercase text-[#6B7280]">
                  Method
                </p>
              ...
              {!!tracking && (
                <div className="mb-3.5">
                  ...
                  <button
                    ...
                    className="mt-1 flex w-full items-center rounded-xl border border-white/7 bg-surface-2 px-3.5 py-3"
                  >
