...
const NAV = [
  { href: "/seller", label: "Dashboard", icon: Grid2x2, match: (p: string) => p === "/seller" },
  { href: "/seller/products", label: "Products", icon: Package, match: (p: string) => p.startsWith("/seller/products") },
  { href: "/seller/orders", label: "Orders", icon: Receipt, match: (p: string) => p.startsWith("/seller/orders") },
  { href: "/seller/messages", label: "Messages", icon: MessageCircle, match: (p: string) => p.startsWith("/seller/messages") },
  { href: "/seller/subscription", label: "Plan", icon: Diamond, match: (p: string) => p.startsWith("/seller/subscription") },
] as const;
...
export default function SellerLayout({ children }: { children: React.ReactNode }) {
  ...
  const refreshBadges = useCallback(async () => {
    try {
      ...
      const [ordersRes, chatsRes] = await Promise.all([
        ...
        fetch(`${API}/chat/conversations`, { headers }).then((r) => r.json()).catch(() => null),
      ]);
      ...
      if (chatsRes?.success && Array.isArray(chatsRes.data)) {
        const total = chatsRes.data.reduce((sum: number, c: {
          unreadCount?: number;
  ...
  }, [isLoaded, user, router, refreshBadges]);
