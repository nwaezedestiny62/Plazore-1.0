...
function topKey(map: Map<string, number>): { key: string; score: number } {
  let key = "";
  let score = 0;
...
function topN(map: Map<string, number>, n = 3): string[] {
  return [...map.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, n)
    .filter(([, s]) => s > 0)
...
function firstNameOnly(raw: string | null | undefined): string | null {
  if (!raw) return null;
  const part = String(raw).trim().split(/\s+/)[0] || "";
  if (part.length < 2 || part.length > 16) return null;
  return part.charAt(0).toUpperCase() + part.slice(1);
}

function softCat(category: string): string {
...
async function buildInterestProfile(
  ...
  for (const ev of events as any[]) {
    ...
    const ageHours =
      (now - new Date(ev.createdAt).getTime()) / (1000 * 60 * 60);
...
async function pickVisual(
  ...
  if (category) {
    ...
    const any: any = await Product.findOne({
      isActive: true,
      category: { $regex: `^${esc}$`, $options: "i" },
      "images.0": { $exists: true },
    })
...
function intensityFromScore(score: number): "strong" | "medium" | "weak" {
  if (score >= 12) return "strong";
  if (score >= MIN_BEHAVIORAL_SCORE) return "medium";
  return "weak";
}
...
async function generatePersonalizedCreative(opts: {
  ...
  const mode =
    topCat.score >= MIN_BEHAVIORAL_SCORE && topCat.key
      ? "behavioral"
      : "cold_start";
  ...
  let primarySignal =
    "Limited behavioral data — using marketplace discovery logic.";
...
export async function resolvePersonalizedSlot(opts: {
  ...
  state = await PersonalizedBannerState.findOneAndUpdate(
    filter,
    { $set: payload },
