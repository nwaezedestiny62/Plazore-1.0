...
import { buildSystemPrompt, buildUserPrompt, PROMPT_VERSION, } from "./prompts.js";
...
function normalizeSpecifications(raw) {
    ...
    if (raw instanceof Map) {
        return Object.fromEntries([...raw.entries()].filter(([, v]) => v != null && String(v).trim().length > 0));
    }
 ...
 */
export async function generateProductAI(productId) {
    ...
    const verificationDocuments = Array.isArray(product.verificationDocuments)
        ? product.verificationDocuments.map((d) => ({
    ...
    try {
        ...
        console.error(`[Plazore AI] Generation failed for ${productId}:`, err.message);
        aiDoc.status = "failed";
