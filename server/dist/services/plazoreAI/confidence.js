 ...
 */
export function calculateBuyerConfidence(input) {
    let score = 0;
    const factors = [];
    ...
    const hasCourier = input.shipping?.method === "courier"
    ...
    if (input.fulfillmentLocation?.countryCode &&
    ...
    const specCount = input.specifications
        ? Object.keys(input.specifications).filter((k) => String(input.specifications[k] || "").trim().length > 0).length
        : 0;
    if (specCount >= 3) {
        score += 8;
    ...
    let sellerScore = 0;
    if (input.seller?.storeName)
        sellerScore += 5;
    if (input.seller?.storeLogo)
        sellerScore += 3;
    if (input.seller?.storeDescription)
        sellerScore += 3;
    if (input.seller?.isSellerVerified)
        sellerScore += 4;
    score += sellerScore;
    if (sellerScore >= 12) {
