...
export const ROOM_CAPACITY = { 1: 50, 2: 14, 3: 16, 4: 33 };
...
function scoreProduct(opts) {
    ...
    const conversion = views > 0 ? (purchases * 3 + carts) / Math.max(views, 1) : 0;
    const commerceRaw = purchases * 15 + carts * 5 + views * 0.4 + wishlist * 2 + conversion * 40;
    ...
    if (exposurePenalty > 0.3)
        reasons.push("exposure_penalty");
    return { product, score, reasons };
}
/** Soft diversity selection */
function selectWithDiversity(scored, capacity, opts = {}) {
    ...
    const tryPick = (relax) => {
        for (const item of scored) {
            ...
            const seller = String(item.product.seller?._id || item.product.seller || "unknown");
            if (!relax) {
                if ((catCount.get(cat) || 0) >= maxPerCategory)
                    continue;
                if ((sellerCount.get(seller) || 0) >= maxPerSeller)
                    continue;
            }
            selected.push(item);
            used.add(id);
...
export async function generateShowroom(opts) {
    ...
    if (session &&
        ...
        const hydrate = (ids) => ids.map((id) => map.get(String(id))).filter(Boolean);
        return {
            sessionId,
    ...
    const baseFilter = { isActive: true, stock: { $gt: 0 } };
    ...
    const room4Pool = candidates.length < 80
        ? scored // allow reuse
    ...
    await ShowroomSession.findOneAndUpdate({ sessionId }, {
        sessionId,
        user: userId || null,
        region,
...
export async function rankProductsForSearch(opts) {
    ...
    const interest = await buildInterestProfile(opts.userId || null, opts.sessionId || "search");
