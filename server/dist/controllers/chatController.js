...
async function notifyChatRecipient(opts) {
    ...
    if (!recipientId || recipientId === sender) {
        console.log("[chat-notify] skip: no recipient or same as sender", {
            recipientId,
            sender,
        });
        return;
    ...
    let productName = "";
    let productId = opts.conversation.product?._id || opts.conversation.product || null;
    ...
    console.log("[chat-notify]", {
        ...
        conversationId: String(opts.conversation._id),
        created: !!created,
        createdId: created?._id ? String(created._id) : null,
    });
}
export const startConversation = async (req, res) => {
    try {
        ...
        const product = await Product.findById(productId).select("seller isActive name images price");
...
export const getMessages = async (req, res) => {
    try {
        ...
        const isParticipant = String(conversation.buyer) === String(user._id) ||
