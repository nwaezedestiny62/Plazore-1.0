...
import { getCtx, resolveExpired } from "../utils/moderation.js";
function ensureModerationShape(user) {
    if (!user.moderation) {
...
async function applyState(user, context, status, opts) {
    ensureModerationShape(user);
    const side = user.moderation[context];
    const prev = side.status || "NORMAL";
    side.status = status;
...
export const getModerationStats = async (_req, res) => {
    try {
        ...
        const [pendingChecks, underReview, sellerSuspensions, buyerSuspensions, sellerBlocks, buyerBlocks, recentlyPardoned, recentlyRestored, recentEvents,] = await Promise.all([
...
export const listModerationCases = async (req, res) => {
    try {
...
export const searchModerationAccounts = async (req, res) => {
    try {
        ...
        const users = await User.find(filter)
            .limit(25)
            .select("name email role storeName image phone marketplaceRegion isSellerSuspended isSellerVerified moderation createdAt updatedAt")
            .lean();
...
export const getModerationProfile = async (req, res) => {
    try {
        let live = await User.findById(req.params.id);
        if (!live) {
        ...
        const [cases, events, reports, products, productCount, orderAsBuyer, orderAsSeller, recentOrdersBuyer, recentOrdersSeller, addresses, paymentMethods, wishlistDoc, savedStoresDoc,] = await Promise.all([
        ...
        const wishlist = (wishlistDoc?.products || []).map((p) => ({
            _id: p?._id,
            name: p?.name,
            price: p?.price,
            isActive: p?.isActive,
        }));
        const savedStores = (savedStoresDoc?.stores || []).map((s) => ({
            _id: s?._id,
...
export const suspendAccount = async (req, res) => {
    try {
        ...
        const endsAt = hours > 0 ? new Date(Date.now() + hours * 60 * 60 * 1000) : undefined;
        const publicReason = context === "seller"
...
export const requestActivityCheck = async (req, res) => {
    try {
        const context = req.body.context;
        if (context !== "buyer" && context !== "seller") {
            return res
        ...
        const publicReason = "We're reviewing your activity to make sure everything is in order.";
        const c = await ModerationCase.create({
            user: user._id,
            context,
            status: "UNDER_REVIEW",
...
export const pardonAccount = async (req, res) => {
    try {
        const context = req.body.context;
        if (context !== "buyer" && context !== "seller") {
            return res
        ...
        const publicReason = "We've completed our review and no further action is required on your account.";
...
export const addModerationNote = async (req, res) => {
    try {
        ...
        const admin = req.user;
        const c = await ModerationCase.findById(req.params.caseId);
        if (!c) {
...
export const getMyModeration = async (req, res) => {
    try {
        let user = req.user;
...
export const clearLastOutcome = async (req, res) => {
    try {
        const context = req.body.context;
        if (context !== "buyer" && context !== "seller") {
            return res
