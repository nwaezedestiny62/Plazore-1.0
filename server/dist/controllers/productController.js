...
const SELLER_PUBLIC_FIELDS = "name storeName storeLogo storeDescription isSellerVerified marketplaceRegion shippingDefaults";
/** Parse structured fulfillment location from form body (multipart or JSON) */
function parseFulfillmentLocation(body) {
...
async function uploadVerificationDocs(files, body) {
    ...
    const results = await Promise.all(files.map(async (file, i) => {
        const secureUrl = await uploadToCloudinary(file.buffer, "plazore/documents", "auto");
        return {
            documentName: String(names[i] || file.originalname || `Document ${i + 1}`).trim(),
...
function parseExistingDocuments(body, fallback = []) {
    if (body.existingDocuments === undefined)
        return fallback;
    try {
        const raw = typeof body.existingDocuments === "string"
...
export const getProducts = async (req, res) => {
    try {
        ...
        let mongoSort = { isFeatured: -1, createdAt: -1 };
        if (sortParam === "newest") {
        ...
        const wantsRank = sortParam === "trending" ||
            sortParam === "relevant" ||
...
export const createProduct = async (req, res) => {
    try {
        ...
        if (imageFiles.length > 0) {
            try {
                images = await Promise.all(imageFiles.map((file) => uploadToCloudinary(file.buffer, "plazore/products", "image")));
            }
            catch (uploadErr) {
                console.error("Cloudinary image upload error:", uploadErr);
                return res.status(502).json({
                    success: false,
                    message: uploadErr?.message?.includes("EAI_AGAIN") ||
        ...
        try {
            verificationDocuments = await uploadVerificationDocs(getDocumentFiles(req), req.body);
        }
        catch (docErr) {
        ...
        const { name, description, price, stock, category, subCategory, brand, shippingMethod, courierCompany, deliveryFee, } = req.body;
        ...
        if (!fulfillmentLocation) {
            return res.status(400).json({
                success: false,
                message: "Fulfillment location is required (country and city — where this product ships from)",
            });
        }
        ...
        const product = await Product.create({
            ...
            shipping: {
                method,
                courierCompany: method === "courier" ? String(courierCompany || "").trim() : "",
                deliveryFee: safeFee,
            },
            fulfillmentLocation,
            specifications,
...
export const updateProduct = async (req, res) => {
    try {
        ...
        if (imageFiles.length > 0) {
            try {
                const newImages = await Promise.all(imageFiles.map((file) => uploadToCloudinary(file.buffer, "plazore/products", "image")));
        ...
        if (req.body.shippingMethod !== undefined ||
            ...
            const method = req.body.shippingMethod === "self"
                ? "self"
                : req.body.shippingMethod === "courier"
                    ? "courier"
                    : product.shipping?.method || "courier";
            const fee = req.body.deliveryFee !== undefined
                ? Number(req.body.deliveryFee)
                : product.shipping?.deliveryFee || 0;
            updates.shipping = {
                method,
                courierCompany: method === "courier"
                    ? String(req.body.courierCompany ??
        ...
        if (req.body.fulfillmentCountryCode !== undefined ||
            ...
            if (!fulfillmentLocation) {
                return res.status(400).json({
                    success: false,
                    message: "Invalid fulfillment location — country and city are required",
                });
            }
        ...
        const isDocEdit = req.body.existingDocuments !== undefined ||
            req.body.specifications !== undefined ||
            getDocumentFiles(req).length > 0 ||
            req.body.name !== undefined;
        if (isDocEdit) {
            const existingDocs = parseExistingDocuments(req.body, product.verificationDocuments || []);
            try {
                const newDocs = await uploadVerificationDocs(getDocumentFiles(req), req.body);
        ...
        if (updated) {
            const onlyVisibility = Object.keys(updates).length === 1 && updates.isActive !== undefined;
            if (!onlyVisibility) {
...
export const deleteProduct = async (req, res) => {
    try {
        ...
        if (product.verificationDocuments?.length) {
            await Promise.all(product.verificationDocuments.map(async (doc) => {
                try {
...
// ======================================================
export const trackShowroomEvent = async (req, res) => {
    try {
        const user = getUser(req);
        ...
        if (!sessionId || !productId || !allowed.includes(eventType)) {
            return res.status(400).json({
                success: false,
                message: "sessionId, productId and type (impression|open|cart|wishlist|purchase|skip) required",
            });
        }
