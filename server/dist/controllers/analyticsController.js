    ...
    try {
        ...
        const series = keys.map((date) => {
            ...
            for (const p of perfs) {
                const row = (p.daily || []).find((x) => x.date === date);
                if (row) {
                    score += row.score || 0;
                    views += row.views || 0;
        ...
        const topProducts = perfs.slice(0, 7).map((p) => ({
...
export const getProductAnalytics = async (req, res) => {
    try {
        ...
        let perf = await ProductPerformance.findOne({ product: id }).lean();
        if (!perf) {
            perf = {
                views: 0,
                cartAdds: 0,
        ...
        const series = keys.map((date) => {
            const row = (perf.daily || []).find((x) => x.date === date);
            return {
                date,
                score: row?.score || 0,
                views: row?.views || 0,
