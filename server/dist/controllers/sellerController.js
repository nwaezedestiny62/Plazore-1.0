...
export const applyAsSeller = async (req, res) => {
    try {
        const user = req.user;
        const { storeName, storeDescription, businessGoal, phone, bankName, accountName, accountNumber, } = req.body;
        if (!storeName?.trim()) {
...
const uploadOne = (file, folder) => new Promise((resolve, reject) => {
...
export const getMyStore = async (req, res) => {
    try {
        ...
        const full = await User.findById(user._id).select("name email phone storeName storeDescription businessGoal storeLogo storeBanner payout shippingDefaults isSellerVerified sellerAppliedAt role");
        if (!full) {
            return res
                .status(404)
    ...
    try {
        ...
        if (shippingDefaults && typeof shippingDefaults === "object") {
            ...
            updates.shippingDefaults = {
                ...
                deliveryMethod: shippingDefaults.deliveryMethod === "self" ||
        ...
        if (files?.storeBanner?.[0]) {
            updates.storeBanner = await uploadOne(files.storeBanner[0], "plazore/store");
        }
        ...
        const updated = await User.findByIdAndUpdate(user._id, updates, {
            new: true,
            runValidators: true,
        }).select("name email phone storeName storeDescription businessGoal storeLogo storeBanner payout shippingDefaults isSellerVerified sellerAppliedAt role");
...
export const getPublicStorefront = async (req, res) => {
    try {
        ...
        const seller = await User.findById(id).select('storeName storeDescription businessGoal storeLogo storeBanner role isSellerVerified name shippingDefaults');
        if (!seller) {
...
export const verifyPayoutAccess = async (req, res) => {
    try {
        const user = req.user;
        if (!user?._id) {
            return res.status(401).json({ success: false, message: "Not authorized" });
        }
        const raw = String(req.body?.lastFour ?? req.body?.last4 ?? req.body?.digits ?? "");
