...
import { generateShowroom, ROOM_CAPACITY, SESSION_TTL_MS, } from "../services/showroomRanker.js";
...
const ALGORITHM = {
    ...
    reusePolicy: "Rooms 1–2 are the primary unique set. Rooms 3–4 may reuse earlier products when inventory is thin (room 3 if candidates < 40, room 4 if candidates < 80), then fill-short as last resort.",
    regionalFallback: "Prefer the selected region. If local eligible inventory is under 120, supplement with other regions.",
};
function asId(v) {
    return String(v || "");
}
function dist(items, keyFn) {
    const map = {};
...
async function hydrateRooms(idsByRoom) {
    ...
    const docs = unique.length
        ? await Product.find({ _id: { $in: unique } })
            ...
            .lean()
        : [];
    const map = new Map(docs.map((p) => [asId(p._id), p]));
    const pick = (ids) => (ids || []).map((id) => compactProduct(map.get(asId(id)))).filter(Boolean);
    return {
        1: pick(idsByRoom[1]),
        2: pick(idsByRoom[2]),
        3: pick(idsByRoom[3]),
...
function roomStats(products, capacity, earlierIds) {
...
export const getAdminShowroom = async (req, res) => {
    try {
        const region = String(req.query.region || "NG").trim() || "NG";
        const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        const baseEligible = { isActive: true, stock: { $gt: 0 } };
        const [totalActive, eligible, localEligible, outOfStock, inactive, recentAdded, recentUpdated, categoryRows, sellerCount, regionRows, latestSession, sessionCount,] = await Promise.all([
            ...
            Product.distinct("seller", baseEligible),
            Product.aggregate([
                { $match: baseEligible },
        ...
        const populatedTotal = sections[1].populated +
            sections[2].populated +
            sections[3].populated +
            sections[4].populated;
        const capacityTotal = ROOM_CAPACITY[1] + ROOM_CAPACITY[2] + ROOM_CAPACITY[3] + ROOM_CAPACITY[4];
        ...
        res.json({
            ...
            data: {
                ...
                categories: categoryRows.map((r) => ({ key: r._id || "—", count: r.n })),
                regions: regionRows.map((r) => ({ key: r._id || "—", count: r.n })),
                algorithm: {
                    ...ALGORITHM,
...
export const refreshAdminShowroom = async (req, res) => {
    try {
...
export const getAdminIntelligence = async (req, res) => {
    try {
        ...
        const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        const [ready, pending, failed, withAi, totalProducts, recentReady, lastReady, confidenceRows,] = await Promise.all([
        ...
        const overview = {
            ...
            lastSuccessfulUpdate: lastReady?.generatedAt || null,
            buyerConfidence: {
                high: confidenceRows.find((r) => r._id === "High Confidence")?.n || 0,
                growing: confidenceRows.find((r) => r._id === "Growing Confidence")?.n || 0,
                limited: confidenceRows.find((r) => r._id === "Limited Confidence")?.n || 0,
            },
        };
        if (status === "missing") {
            const aiIds = await ProductAI.distinct("productId");
            const productFilter = { _id: { $nin: aiIds } };
            if (q) {
                productFilter.$or = [
        ...
        const aiFilter = {};
        if (status)
            aiFilter.status = status;
        if (confidence)
            aiFilter["buyerConfidence.level"] = confidence;
        if (q) {
        ...
        const pids = aiItems.map((i) => i.productId);
        const products = await Product.find({ _id: { $in: pids } })
            .populate("seller", "name storeName email")
            .lean();
        const pmap = new Map(products.map((p) => [asId(p._id), p]));
        res.json({
            success: true,
            data: {
                overview,
                items: aiItems.map((row) => {
...
export const getAdminIntelligenceDetail = async (req, res) => {
    try {
        ...
        const product = await Product.findById(id)
            .populate("seller", "name storeName email isSellerVerified isSellerSuspended marketplaceRegion createdAt")
            .lean();
        if (!product) {
        ...
        res.json({
            ...
            data: {
                source: {
                    ...
                    fulfillmentLocation: product.fulfillmentLocation || null,
...
export const regenerateAdminIntelligence = async (req, res) => {
    try {
        const id = String(req.params.productId || "").trim();
        if (!mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ success: false, message: "Invalid product id" });
        ...
        await ProductAI.updateOne({ productId: id }, { $set: { status: "pending", error: "" } });
        enqueueProductAI(id);
...
function fillDays(keys, byDay, extra) {
...
export const getAdminAnalytics = async (req, res) => {
    try {
        ...
        const [gmvAgg, completed, cancelled, refunded, failedPayments, orderTotal, aovAgg, cartsWithItems, cartDocs, orderSeries, sellersTotal, sellersNew, productsListed, usersTotal, buyers, sellerAccounts, usersNew, activeUsers, repeat, presence, showroomFunnel, aiReady, aiFailed, aiPending, perfTotals,] = await Promise.all([
            Order.aggregate([
        ...
        const seriesMap = new Map((orderSeries || []).map((r) => [r._id, r]));
        ...
        const presenceMap = { web: 0, app: 0, admin: 0, unknown: 0 };
        for (const r of presence || []) {
            const k = r._id || "unknown";
            if (k === "web" || k === "app" || k === "admin")
                presenceMap[k] = r.n;
            else
                presenceMap.unknown += r.n;
        }
