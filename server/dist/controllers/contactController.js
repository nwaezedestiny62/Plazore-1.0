...
function assertWords(text, max = 300) {
    const n = wordCount(text);
    if (n < 1)
        throw new Error("Message is required");
    if (n > max) {
        throw new Error(`Message must be ${max} words or fewer (you wrote ${n})`);
    }
}
...
export const createContact = async (req, res) => {
    try {
        ...
        const { contactAs, contextType = "general", category = "other", subject = "", message, email, country, state, city, street = "", productId, storeId, orderId, } = req.body || {};
        ...
        let role = contactAs === "seller" ? "seller" : "buyer";
        if (user.role === "seller" && contactAs !== "buyer")
            role = "seller";
        if (user.role === "buyer")
            role = "buyer";
...
export const createReport = async (req, res) => {
    try {
        ...
        const { targetType, productId, storeId, reason, description, email, country, state, city, street = "", } = req.body || {};
        ...
        try {
            report = await Report.create({
                reporter: user._id,
                reporterRole: (user.role === "seller" ? "seller" : "buyer"),
...
export const myContacts = async (req, res) => {
    try {
        const user = req.user;
        if (!user?._id) {
            return res.status(401).json({ success: false, message: "Unauthorized" });
...
export const myContactDetail = async (req, res) => {
    try {
        const user = req.user;
        if (!user?._id) {
            return res.status(401).json({ success: false, message: "Unauthorized" });
        ...
        await ContactMessage.updateOne({ _id: item._id }, { $set: { unreadByUser: false } }).catch(() => { });
...
export const replyMyContact = async (req, res) => {
    try {
        const user = req.user;
        if (!user?._id) {
            return res.status(401).json({ success: false, message: "Unauthorized" });
        ...
        item.status = "awaiting_plazore";
        item.unreadByAdmin = true;
        item.unreadByUser = false;
