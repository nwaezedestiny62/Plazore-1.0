...
/**
 * Structured geo data for fulfillment pickers.
 * Expand over time without changing the product schema.
 */
    ...
    states: [
      {
        code: "LA",
        name: "Lagos",
        cities: ["Lagos", "Ikeja", "Lekki", "Surulere", "Yaba", "Ajah"],
      },
      {
        code: "FC",
        name: "FCT",
        cities: ["Abuja", "Gwarinpa", "Wuse", "Maitama"],
      },
      {
        code: "RI",
        name: "Rivers",
    ...
    states: [
      {
        code: "NY",
        name: "New York",
        cities: ["New York", "Brooklyn", "Buffalo"],
      },
      {
        code: "CA",
        name: "California",
...
export function getCitiesForState(countryCode: string, stateCode: string) {
  const state = getStatesForCountry(countryCode).find((s) => s.code === stateCode);
  return state?.cities || [];
}